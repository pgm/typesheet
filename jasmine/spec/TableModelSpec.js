describe("TableController", function() {
    ...
    it("clicking on empty cell results in adding element", function() {
        ...
        expect(elements).toEqual([{type: "editor", value: ""}])

        c.updateEditorValue("value");
        c.acceptEditorValue();

        expect(c.state.uncommitted).toEqual([{op: "AV", instance: "a", property:"x", value:"value"}]);

        var elements = getCellElements(0, 0, c.state.uncommitted, c.state.pending,  c.state.data[0][0], c.state.editorState);
        expect(elements).toEqual([{type: "uncommitted", value: "value"}])
    });
    ...
    it("clicking on existing cell results in remove and add", function() {
        ...
        var elements = getCellElements(0, 0, c.state.uncommitted, c.state.pending, c.state.data[0][0], c.state.editorState);
        expect(elements).toEqual([{type: "editor", value: "0"}])
        ...
        var elements = getCellElements(0, 0, c.state.uncommitted, c.state.pending, c.state.data[0][0], c.state.editorState);
        expect(elements).toEqual([{type: "uncommitted", value: "1"}])
    });
    ...
    it("clicking on plus on a cell with a value results in add", function() {
        ...
        var elements = getCellElements(0, 0, c.state.uncommitted, c.state.pending, c.state.data[0][0], c.state.editorState);
        expect(elements).toEqual([{type: "data", value: "0"},{type: "editor", value: ""}])

        c.updateEditorValue("1");
        c.acceptEditorValue();
        expect(c.state.uncommitted).toEqual([{op: "AV", instance: "a", property:"x", value:"1"}])

        var elements = getCellElements(0, 0, c.state.uncommitted, c.state.pending, c.state.data[0][0], c.state.editorState);
        expect(elements).toEqual([{type: "data", value: "0"},{type: "uncommitted", value: "1"}])
    });
    ...
    test add/del with value in pending
});
